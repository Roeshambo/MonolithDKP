...
function MonDKP:ErrantCheck(req) -- param = request sync when completed
	...
	for k1,v1 in pairs(MonDKP_Meta) do
		for k2,v2 in pairs(v1) do
			if not core.ArchiveActive then
				if MonDKP_Meta[k1][k2] and MonDKP_Meta[k1][k2].lowest > 1 then  -- ensures "lowest" values are set to 1 if they are somehow set higher when no archive exists
					MonDKP_Meta[k1][k2].lowest = 1  	-- lowest set to 1 because if no archvie exists, a 1st index must exist. (first entry to go into the archive would be 1. So they are mutually exclusive)
				end
			elseif core.ArchiveActive then
				if (MonDKP_Archive_Meta[k1][k2] and MonDKP_Archive_Meta[k1][k2]) and MonDKP_Meta[k1][k2].lowest < MonDKP_Archive_Meta[k1][k2] then
					MonDKP_Meta[k1][k2].lowest = MonDKP_Archive_Meta[k1][k2] + 1 	-- sets "lowest" to 1 above current archive index
				end
			end
		end
	end
	...
	for k,v in pairs(MonDKP_Meta.DKP) do
		...
		CheckCount = CheckCount + 1   -- counts cycles running. When reaches back to 0, all cycles completed

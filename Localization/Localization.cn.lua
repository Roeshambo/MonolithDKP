if GetLocale() == "zhCN" then
  ...
  core.L = {
    ...
    AUTOAWARD             = "汽车奖DKP",
    AUTOAWARDTTDESC       = "杀死老板时自动奖励Boss Kill Bonus DKP（在“选项”选项卡中设置）。",
    AWARDBONUS            = "奖励奖金",
    ...
    CHANGELOG1            = "- 其他优化措施可减少广播延迟。",
    CHANGELOG2            = "- 优化的DKP调整可大大减少执行时间。",
    CHANGELOG3            = "- 过滤器现在可以与所有列表视图（raid，standby，core等）一起正常使用",
    CHANGELOG4            = "- 在“ DKP历史记录”选项卡中添加了过滤器下拉列表，以查看个人的完整DKP历史记录。",
    CHANGELOG5            = "- 修复了！dkp命令，该命令在仅发送！dkp时无法正确搜索用户的问题。",
    CHANGELOG6            = "- 在“ DKP模式”窗口中添加了“待机选择加入”选项。处于活跃状态时，首领被杀死后，突袭首领将向公会宣布，未参加突袭的任何人都可以有2分钟的时间选择等待。不在空袭中的玩家将在GUILD聊天中使用！standby（或备用名称，如果是！standby NAME）来加入。此备用列表在每次杀死首领时都会清除。这是为了仅奖励那些当前处于活动状态的备用设备。",
    CHANGELOG7            = "- 在“ DKP模式”窗口中添加了“自动奖励DKP”选项。老板被杀死后，这将自动奖励整个团队（和待机，如果在同一“模式”窗口中选择）。如果启用了“待机选择加入”，它将立即授予突袭，并在每个首领2分钟的“选择加入”窗口到期后授予待机列表中的成员。只有突袭首领会分配分数。如果突袭首领没有此附加组件，则不会传播任何积分。这是为了防止多名官员应用奖金。",
    CHANGELOG8            = "",
    CHANGELOG9            = "",
    CHANGELOG10           = "",
  }

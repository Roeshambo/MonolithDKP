if GetLocale() == "zhCN" then
  ...
  core.L = {
    ...
    CHANGELOG1            = "- 在主GUI中添加了搜索框。搜索并返回按名称，类别，规格，等级和角色找到的结果。",
    CHANGELOG2            = "- 该表现在可以按规格及其他排序。",
    CHANGELOG3            = "- 改进的shift / ctrl单击功能可提供更多功能和更自然的感觉。 Shift + Click现在将在您的上一个选择和新选择之间进行选择。 Ctrl / Cmd + Click将包含单个选择，而不会删除先前的选择。",
    CHANGELOG4            = "- 如果其中一名警官重新装填或当机，突袭计时器现在将尝试与没有计时器运行的警官进行同步。仅在团队中有一名或多名其他人员进行广播时才能工作。",
    CHANGELOG5            = "- 突袭计时器现在有一个弹出按钮，可让您使用一个小的可移动窗口来显示突袭计时器。",
    CHANGELOG6            = "- 将数据广播到行会时，确定了延迟的来源。 DKP历史记录选项卡已从一次生成50个条目减少到了5个（使用按钮可进一步扩展该视图）。",
    CHANGELOG7            = "- 您的本地化不正确吗？请访问https://www.curseforge.com/wow/addons/monolith-dkp/localization/以帮助提供适当的翻译！",
    CHANGELOG8            = "",
    CHANGELOG9            = "",
    CHANGELOG10           = "",
  }

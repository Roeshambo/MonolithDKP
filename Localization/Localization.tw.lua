if GetLocale() == "zhTW" then
  ...
  core.L = {
    ...
    AUTOCOMBATLOGTTDESC = "在團隊中進入團隊區域或在遭遇團隊時進入戰鬥時自動激活/ combatlog。",
    ...
    AWARDINTERVALTTWARN = "輸入60將在每小時上方的“獎勵”欄位中獎勵RAID（和待機，如果在下麵選中的話）DKP。",
    AWARDITEM = "獎勵項目",
    AWARDITEMTO = "獎勵項目",
    ...
    Ex：|cffa335ee|Hitem:16865::::::::120:577::::::|h[Breastplate of Might]|h|r下降，且最低出價為50 DKP。每個想要的玩家都通過一個打開的通道（在下面選擇）發送“！bid 50”（或更高），而不超過其可用的DKP（除非激活了低於零的DKP）。出價最高的玩家將獲勝，並被收取最低DKP值或第二高出價者提交的值。閱讀工具提示以獲取更多資訊。]=],
    ...
    POINTS = "點數",
    POINTSTTDESC = "在DKP表中輸入分配給選定玩家的DKP數量。預設值可以在下麵的“選項”選項卡中更改。",
    ...
    SETWHITELISTTTWARN = "只有軍官才能被列入白名單。此選項專門用於限制在此插件中哪些軍官具有軍官權限。如果您希望所有幹部都獲得許可，請將白名單留空。",
    SHADOW = "暗影",
    SHOULDERS = "護肩",
    ...
    Ex：|cffa335ee|Hitem:19143::::::::120:577::::::|h[Flameguard Gauntlets]|h|r下降，靜態值為50 DKP。每個想要的玩家都通過一個打開的通道（在下面選擇）使用"！bid "。具有最高可用DKP的播放器獲勝，並收取該靜態值。閱讀工具提示以獲取更多資訊。]=],
    ...
    CHANGELOG4            = "-軍官可以使用表格維修功能。要使用它，請觀看“詛咒”頁面上鍊接的視頻或轉到https://www.youtube.com/watch?v=dwnNnppFF2I。建議只有一名軍官在數據最準確的表格上使用它，然後將修復後的數據完整廣播到其他行會或軍官，以便傳遞。事先備份您保存的變量文件。",

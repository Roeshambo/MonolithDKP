...
core.L = {
  ...
  LIMITTOZONE           = "Include only players in zone",
  LIMITTOZONETTDESC     = "Selecting this will only distribute DKP to players in the same zone as the distributor. This does not affect players that are on standby.",
  ...
  CHANGELOG3            = "- Filters now work properly with all list views (raid, standby, core etc.)",
  CHANGELOG4            = "- Added filter dropdown to DKP History tab to view an individuals complete DKP history. (new additions requires a /reload)",
  CHANGELOG5            = "- Fixed !dkp command that was not searching user properly when just !dkp was sent.",
  CHANGELOG6            = "- Added Standby Opt In option to DKP Modes window. When active, after a boss is killed, the raid leader will announce to guild that anyone not in the raid has 2 minutes to opt in as a standby. Players not in the raid will use !standby (or !standby NAME if on an alt) in GUILD chat to opt in. This standby list will be cleared on every boss kill. This is to only award those on standby that are currently active.",
  CHANGELOG7            = "- Added Auto Award DKP option to DKP Modes window. This will automatically award the entire raid (and standby, if selected in the same Modes window) Boss Kill DKP as soon as a boss is killed. If Standby Opt In is active, it will award the raid immediately, and award those on the standby list after the 2 minute Opt In window expires after each boss. ONLY the raid leader will distribute points. If the raid leader does not have this addon, no points will be disseminated. This is to prevent multiple officers applying the bonus.",
  CHANGELOG8            = "",
  CHANGELOG9            = "",
  CHANGELOG10           = "",
}
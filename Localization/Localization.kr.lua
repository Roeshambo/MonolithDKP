if GetLocale() == "koKR" then
  ...
  core.L = {
    ...
    AUTOAWARD             = "자동 수상 DKP",
    AUTOAWARDTTDESC       = "보스를 처치하면 보스 처치 보너스 DKP (옵션 탭에서 설정)가 자동으로 부여됩니다.",
    AWARDBONUS            = "보너스 보너스",
    ...
    CHANGELOG4            = "- 개인의 완전한 DKP 기록을 볼 수 있도록 DKP 기록 탭에 필터 드롭 다운을 추가했습니다.",
    CHANGELOG5            = "- ! dkp 만 전송되었을 때 사용자를 제대로 검색하지 못하는! dkp 명령을 수정했습니다.",
    CHANGELOG6            = "- DKP 모드 창에 대기 옵트 인 옵션을 추가했습니다. 활성화되면 보스를 처치 한 후 레이드 리더는 레이드에 참여하지 않은 사람은 대기 시간으로 2 분을 선택했다고 길드에 발표합니다. 레이드에 참여하지 않은 플레이어는 GUILD 채팅에서! standby (또는 alt에있는 경우! standby NAME)를 사용하여 옵트 인합니다.이 대기 목록은 모든 보스 처치시 지워집니다. 이것은 현재 활성 상태 인 대기자에게만 수여됩니다.",
    CHANGELOG7            = "- DKP 모드 창에 자동 보너스 DKP 옵션이 추가되었습니다. 보스가 죽 자마자 전체 공격대 (동일한 모드 창에서 선택한 경우 대기)를 자동으로 수여합니다. 대기 옵트 인이 활성화 된 경우 즉시 레이드를 부여하고 각 보스 후 2 분의 옵트 인 창이 만료 된 후 대기 목록에있는 공격자를 보상합니다. 레이드 리더 만 포인트를 분배합니다. 공격 대장이이 애드온을 가지고 있지 않으면 점수가 전파되지 않습니다. 이것은 여러 장교가 보너스를 적용하지 못하도록하기위한 것입니다.",
    CHANGELOG8            = "",
    CHANGELOG9            = "",
    CHANGELOG10           = "",
  }
